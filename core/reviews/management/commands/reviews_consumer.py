...
class Command(BaseCommand):
    help = 'Starts the RabbitMQ consumer for reviews service'
    ...
    def handle_message(self, message):
        ...
        if event_type == 'category_updated':
            self.stdout.write(
                self.style.SUCCESS(
                    f'📝 Category updated in Reviews service:\n'
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'🆕 New category available in Reviews:\n'
                    f'   ID: {message.get("category_id")}\n'
                    f'   Name: {message.get("name")}\n'
                    f'   Description: {message.get("description")}'
                )
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('🚀 Starting Reviews Consumer...'))
        
        handler = RabbitMQHandler()
        
        try:
            exchange_name = 'category_events'
            queue_name = 'reviews.events'
