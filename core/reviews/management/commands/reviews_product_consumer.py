# reviews/management/commands/reviews_product_consumer.py
...
class Command(BaseCommand):
    help = 'Starts the RabbitMQ consumer for product events in Reviews service'
    ...
    def handle_message(self, message):
        ...
        if event_type == 'product_updated':
            self.stdout.write(
                self.style.SUCCESS(
                    f'📝 Product updated in Reviews service:\n'
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'🗑️ Product deleted from Reviews service:\n'
                    f'   ID: {message.get("product_id")}'
                )
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'✨ New product available for Reviews:\n'
                    f'   ID: {message.get("product_id")}\n'
                    f'   Name: {message.get("name")}\n'
                    f'   Description: {message.get("description")}'
                )
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('🚀 Starting Reviews Product Consumer...'))
        
        handler = RabbitMQHandler()
        
        try:
            exchange_name = 'product_events'
            queue_name = 'reviews.product_events'
            routing_key = 'product.*'
