...
class Command(BaseCommand):
    help = 'Starts the RabbitMQ consumer for products service'
    ...
    def handle_message(self, message):
        ...
        if event_type == 'category_updated':
            self.stdout.write(
                self.style.SUCCESS(
                    f'📝 Category updated in Products service:\n'
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'🆕 New category available in Products:\n'
                    f'   ID: {message.get("category_id")}\n'
                    f'   Name: {message.get("name")}\n'
                    f'   Description: {message.get("description")}'
                )
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('🚀 Starting Products Consumer...'))
        
        handler = RabbitMQHandler()
        
        try:
            exchange_name = 'category_events'
            queue_name = 'products.events'
            routing_key = 'category.*'  # Make sure this matches
            
            # Add debug logging
            logger.info(f"Setting up consumer with exchange={exchange_name}, queue={queue_name}, routing_key={routing_key}")
            ...
            logger.info('👂 Listening for category events...')
